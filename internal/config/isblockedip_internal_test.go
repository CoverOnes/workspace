...
// isblockedip_internal_test.go — white-box tests for isBlockedIP, the boot-time SSRF
// predicate. isBlockedIP is unexported, so these tests live in package config
// (not config_test).
...
func TestIsBlockedIP(t *testing.T) {
	...
	tests := []struct {
		...
		// RFC1918 — only blocked when blockPrivate is set (production/staging).
		{name: "rfc1918 blocked when blockPrivate", ip: "10.0.0.5", blockPrivate: true, wantBlocked: true, wantReason: "private/RFC1918 address in production"},
